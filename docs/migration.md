...
独立仓库新增workspace.py，用COSMOS_REPO显式定位外部框架，调整launcher、wrapper与waiter的目录解析。输出留在新仓库runs/validation，metadata分别记录框架与工具Git SHA。修复新目录下waiter首次运行时validation目录不存在的问题，并统一本仓库脚本格式；FlagScale报告源码保持原字节和许可证。
